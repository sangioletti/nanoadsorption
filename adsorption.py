...
class MultivalentBinding:
    ...
    def K_LR(self, h, K_bind_0):
        ...
        If binder_linear_size b > 0, the polymer end must reach h_eff = h - b
        for the binder to bridge the remaining gap. P_z is evaluated at h_eff,
        g at h (polymer confined by opposing surface, not binder).
        When h_eff < 0, the binder doesn't fit: K_LR = 0.

        Gaussian/Flory: K_bind_0 * P_z(h_eff) / g(h).
        Flory-exact: K_bind_0 * P_z(h_eff) / g(h) from des Cloizeaux SAW."""
        binder_size = self.binder_linear_size
        h_eff = h - binder_size
        ...
        if self.polymer_model in [ "gaussian", "Flory-approx" ]:
            ...
            if g_lig < 1e-15:
                g_lig = 1e-15
            return K_bind_0 * self._klr_prefactor_factor * np.exp(-h_eff**2 * self._klr_exp_factor) / g_lig
        elif self.polymer_model == "Flory-exact":
            pz = float(self._klr_pz_interp(h_eff))
    ...
    def W_steric(self, h, verbose = False):
        ...
        Returns infinite repulsion when h < binder_linear_size,
        since the binder cannot physically fit in the gap."""
        ...
        if self.binder_linear_size > 0 and h < self.binder_linear_size:
            return np.inf
    ...
    def calculate_binding_constant( self, K_bind_0,
            ...
            z_min = self.binder_linear_size

            if self.binding_model == "saddle":
                N_long = self.data_polymers["ligands"]["N"]
                a = self.data_polymers["ligands"]["a"]
                z_max = N_long * a + self.binder_linear_size
